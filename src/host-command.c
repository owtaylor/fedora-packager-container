...
gboolean
sync_window_size(SyncWindowSizeData *data)
{
  struct winsize w;
  ioctl(data->in_fd, TIOCGWINSZ, &w);
  ioctl(data->out_fd, TIOCSWINSZ, &w);
  ...
  if (flags & PEGG_HOST_COMMAND_USE_PTY)
    {
      ...
      *stderr_handle = add_pty_fd (fd_list, pty_path, O_WRONLY | O_NOCTTY, error);
      ...
      SyncWindowSizeData sync_window_size_data = { 0, pty_master_fd };
      g_unix_signal_add_full (G_PRIORITY_DEFAULT,
                              SIGWINCH,
                              (GSourceFunc)sync_window_size,
                              g_memdup (&sync_window_size_data, sizeof(sync_window_size_data)),
                              (GDestroyNotify)g_free);
      sync_window_size(&sync_window_size_data);
                        ...
                        GError                  **error)
{
  ...
  g_return_val_if_fail (!extra_environment ||
                        g_strv_length (extra_environment) %2 == 0, -1);
  ...
  if (extra_environment)
    {
      char **p;
      for (p = extra_environment; *p; p += 2)
        g_variant_builder_add (env_builder, "{ss}", p[0], p[1]);
    }
